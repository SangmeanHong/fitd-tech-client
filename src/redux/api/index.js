...
const API = axios.create({ baseURL: `http://localhost:${process.env.REACT_APP_PORT}` });
...
const apiGetUsers = async () => {
    return await API.post('/api/user/userlist', { "option": "all" }, {
        withCredentials: true,
    })
}

const apiApplications = async () => {
    return await API.post('/api/user/getApplication', {}, {
        withCredentials: true,
    })
}
